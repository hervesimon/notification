...
class UsersControllerTest {
    ...
    void when_user_technical_issue_then_error_500() throws Exception {
        ...
        when(userService.getUserName("johndoe")).thenThrow(new FunctionalException("notification.error","une erreur est arrivée"));
    ...
    void when_username_trips_request_then_OK() throws Exception {
        User expectedUser = generator.nextObject(User.class);
        when(userService.getUserName("johndoe")).thenReturn(expectedUser);
        this.mockMvc.perform(get(GET_USER_BY_USERNAME_URL, "johndoe")
                    .contentType(MediaType.APPLICATION_JSON)
                    .characterEncoding("UTF8"))
